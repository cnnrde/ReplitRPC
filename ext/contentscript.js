;(function () {
  function inject() {
    ...
    script.text = ''
    script.text += `const oldURLsCheckServer = async () => {try{ const data = await (await fetch("http://localhost:51337")).text(); if(data !== "null"){console.log("We have detected that the ReplitRPC app is not running, the rich presence won't work without it. Download it here: https://replitrpc.repl.co/app/")}}catch{console.log("We have detected that the ReplitRPC app is not running, the rich presence won't work without it. Download it here: https://replitrpc.repl.co/app/")}};`
